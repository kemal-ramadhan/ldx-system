...
const form = useForm({
    ...
    total_units: props.rack.total_units,
    power_capacity: props.rack.power_capacity,
    weight_capacity: props.rack.weight_capacity,
    description: props.rack.description,
    status: props.rack.status,
});

const updateRack = () => {
    form.put(`/admin/racks/${props.rack.id}`);
};
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <h1 class="text-xl font-bold">Edit Rack</h1>
        <!-- Add your form fields here -->
        <Form @submit.prevent="updateRack" v-slot="{ errors, processing }" class="flex flex-col gap-3">
